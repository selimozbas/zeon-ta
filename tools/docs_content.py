...
CONTENT: dict[str, Doc] = {
    ...
    "supertrend": {
        "title": "SuperTrend",
        "formula": (
            "Basic Upper Band = source + multiplier x ATR(period); Basic Lower Band = source - "
            "multiplier x ATR(period), where source defaults to hl2 = (High+Low)/2 and can instead "
            "be hlc3 = (High+Low+Close)/3, hlcc4 = (High+Low+2xClose)/4, or ohlc4 = "
            "(Open+High+Low+Close)/4; Final Upper Band trails downward only, Final Lower Band trails "
            "upward only; SuperTrend = Final Lower Band while price closes above it (uptrend), Final "
            "Upper Band while price closes below it (downtrend); a flip occurs when close crosses to "
            "the opposite band"
        ),
        "about": (
        ...
        "reading": (
            ...
            "colours. `source` swaps which price the bands are centered on — `hlc3` (typical price) "
            "and `hlcc4` (weighted close) lean the bands slightly toward the close instead of the "
            "midrange; `ohlc4` additionally folds in the open."
        ),
        "pitfalls": (
            ...
            'filter such as [adx](adx.md). Only `source="hl2"` (the default) keeps the original, '
            "unsuffixed column names (`SUPERT_10_3.0`); any other `source` appends itself to every "
            "column name (`SUPERT_10_3.0_hlc3`), and `ohlc4` additionally requires passing `open`."
        ),
        "example": [
            ...
            lambda df: zeonta.supertrend(
                df["high"], df["low"], df["close"], length=10, multiplier=3, source="hlc3"
            )[["SUPERT_10_3.0_hlc3", "SUPERTd_10_3.0_hlc3"]].tail(3),
        ],
    },
    "adx": {
        "title": "ADX / DMI",
