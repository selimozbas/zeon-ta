...
- Adding one or a few new indicators outside the planned
  ...
  any *existing* indicator's default behavior or output. A planned
  sourcing batch (TA-Lib's non-candlestick set, MQL5/cTrader's community
  indicators, and so on) stays **minor**, same as always; this exception
  is specifically for the smaller, unscheduled additions that come up
  between those batches. The same exception covers adding a new
  backward-compatible optional parameter to an *existing* indicator when
  its default reproduces the exact prior behavior and output names
  unchanged — e.g. `supertrend()` gaining a `source` parameter, where
  `source="hl2"` (the default) keeps the original, unsuffixed columns —
  the same "nothing existing actually changes" test the bullet above
  already applies to new indicators.
